...
export function useParams() {
  ...
  const hasQuery = router.query.filter;
  if (hasQuery) {
  ...
  function setParams(param) {
    ...
    if (!param) {
      ...
      return router.push(`/blog`, undefined, {
        shallow: true,
      });
    }
    ...
    router.push(qs ? `/blog/?filter=${qs}` : `/`, undefined, {
      shallow: true,
    });
...
export function getDocumentLink(page, slug) {
  return `https://www.eytyy.com/${page}/${slug}`;
}
